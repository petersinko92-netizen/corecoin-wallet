...
export default function AdminTransactionsPage() {
   ...
   const refundUser = async (tx: any) => {
      ...
      // 1. Get current (Casting to 'any' to fix Vercel Build Error)
      ...
      // Force TypeScript to allow indexing with [col]
         ...
         <div className="bg-white rounded-2xl shadow-sm border overflow-hidden text-black">
            <table className="w-full text-left text-sm">
               <thead className="bg-zinc-50 border-b">
                  <tr>
                     <th className="p-4 font-bold text-zinc-400 uppercase text-[10px]">Time</th>
                     <th className="p-4 font-bold text-zinc-400 uppercase text-[10px]">User</th>
                     <th className="p-4 font-bold text-zinc-400 uppercase text-[10px]">Asset</th>
                     <th className="p-4 font-bold text-zinc-400 uppercase text-[10px]">Amount</th>
                     <th className="p-4 font-bold text-zinc-400 uppercase text-[10px]">Network Info</th>
                     <th className="p-4 font-bold text-zinc-400 uppercase text-[10px] text-right">Actions</th>
                  </tr>
               </thead>
                     ...
                     <tr key={tx.id} className="hover:bg-zinc-50/50 transition-colors">
                        ...
                        <td className="p-4 font-medium">
                           <div className="flex flex-col">
                              <span className="text-slate-900">{tx.profiles?.full_name || 'System'}</span>
                              <span className="text-[10px] text-zinc-400 font-mono">{tx.profiles?.email || tx.user_id.slice(0, 12)}</span>
                           </div>
                        </td>
                        ...
                        <td className="p-4 text-right">
                           {tx.status === 'processing' ? (
                              ...
                              <span className={`text-[10px] font-bold uppercase px-3 py-1 rounded-full ${tx.status === 'completed' ? 'bg-zinc-100 text-zinc-500' : 'bg-red-50 text-red-500'}`}>
                                 {tx.status}
                              </span>
                           )}
                        </td>
...
function FilterBtn({ label, active, onClick }: any) {
   return (
      <button
         onClick={onClick}
         className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${active ? 'bg-slate-900 text-white shadow-md' : 'hover:bg-zinc-50 text-zinc-500'}`}
      >
         {label}
      </button>
   )
